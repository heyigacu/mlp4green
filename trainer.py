...
class train_mlp(object):
    ...
    ##############    
    @staticmethod
    def train_bi_classify_kfolds(model, kfolds=None, max_epochs=500, patience=10, save_folder=PWD+'/pretrained/',save_name='mlp_bi.pth', lr=0.001, weight_decay=0):
        val_metrics = []
        val_epochs = []
        for train_loader,val_loader in kfolds:
            ...
            for epoch in range(1, max_epochs+1):
                ...
                early_stopping(epoch, loss_val, model)
        ...
        print(val_epochs)
        return np.array(val_metrics).mean(0), np.array(val_epochs).mean(0)
    @staticmethod
    def train_bi_classify_all(model, all=None, best_epochs=None, patience=10, save_folder=PWD+'/pretrained/',save_name='mlp_bi.pth', lr=0.001, weight_decay=0):
        if torch.cuda.is_available():
            device = torch.device('cuda:0')
        else:
            device = "cpu"
        ...
        rst = []
        for epoch in range(1, int(best_epochs)+1):
            print(epoch)
            loss_train = 0.
            acc_train=0.
            auc_train=0.
    ...
    @staticmethod
    def train_multi_classify_all(model, all=None, max_epochs=500, patience=10, save_folder=PWD+'/pretrained/',save_name='mlp_multi.pth'):
        if torch.cuda.is_available():
            device = torch.device('cuda:0')
        else:
            device = "cpu"
    ...
    @staticmethod
    def test_multi_classify(model, test=None, plot_auc_curve=True, plot_confuse_matrix=True, save_path=PWD+'/pretrained/gnn.pth', savename='GNN', classnames=['fruity', 'floral', 'woody', 'earthy', 'minty', 'musty', 'Odorless', 'pungent', 'leafy', 'burnt', 'animal', 'odorless', 'medicinal', 'alcoholic', 'sour', 'fishy']):
        state_dict = torch.load(save_path)
        model.load_state_dict(state_dict)
        model.eval()
        for i in list(test):
